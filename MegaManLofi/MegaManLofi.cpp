...
void LoadAndRun( const shared_ptr<ConsoleBuffer> consoleBuffer )
{
   ...
   // game objects
   auto entityFactory = shared_ptr<EntityFactory>( new EntityFactory( gameDefs->EntityDefs, uniqueNumberGenerator, clock, random ) );
   ...
   auto playingStateInputHandler = shared_ptr<PlayingStateInputHandler>( new PlayingStateInputHandler( keyboardInputReader, game, game ) );
